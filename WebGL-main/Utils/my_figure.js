...
function CreateSurfaceDataWithIndices(stepsU = 30, stepsV = 30) {
    ...
    for (let u = 0; u <= stepsU; u++) {
        for (let v = 0; v <= stepsV; v++) {
            ...
            const scale = 1.0; // Змініть значення, щоб налаштувати повторення текстури
            texCoords.push((u / stepsU) * scale, (v / stepsV) * scale);
...
function Model(name) {
    ...
    this.BufferData = function(vertices, indices, normals, texCoords) {
        // Validate lengths
...
function initGL() {
    ...
    normalTexture = loadTexture(gl, 'textures/PavingStones142_1K-PNG/PavingStones142_1K-PNG_NormalGL.png');
    diffuseTexture = loadTexture(gl, 'textures/PavingStones142_1K-PNG/PavingStones142_1K-PNG_Color.png');
    roughnessTexture = loadTexture(gl, 'textures/PavingStones142_1K-PNG/PavingStones142_1K-PNG_Roughness.png');
    ...
    shProgram.normalTextureLocation = gl.getUniformLocation(prog, 'normalTexture'); // Уніформа для текстури нормалей
    shProgram.iTexCoord = gl.getAttribLocation(prog, "texCoord");
    shProgram.rotationPointLocation = gl.getUniformLocation(prog, "rotationPoint");
    shProgram.rotationAngleLocation = gl.getUniformLocation(prog, "rotationAngle");

    shProgram.modelMatrixLocation = gl.getUniformLocation(prog, 'ModelMatrix');

    // Create surface model and buffer data
    surface = new Model('Surface');
    const { vertices, indices, normals, texCoords} = CreateSurfaceDataWithIndices();
    surface.BufferData(vertices, indices, normals, texCoords);
...
function drawRotationSphere() {
    const rotationCoords = ConicalEdgeVertex(rotationPoint.v, rotationPoint.u);

    const { vertices, indices } = createSphere(0.1, 10, 10); // Куля радіусом 0.1
    const translatedVertices = [];

    // Переміщуємо кулю в точку обертання
    for (let i = 0; i < vertices.length; i += 3) {
        translatedVertices.push(
            vertices[i] + rotationCoords[0],
            vertices[i + 1] + rotationCoords[1],
            vertices[i + 2] + rotationCoords[2]
        );
    }

    // Завантаження даних кулі в буфери
    ...
    // Встановлюємо однотонний колір кулі без текстури
    gl.uniform1i(shProgram.useTexture, false); // Вимикаємо використання текстури
    gl.uniform4fv(shProgram.iColor, [0.0, 0.0, 1.0, 1.0]); // Синій колір кулі

    // Малюємо кулю
    gl.drawElements(gl.TRIANGLES, indices.length, gl.UNSIGNED_SHORT, 0);

    // Повертаємося до використання текстури для інших об'єктів
    gl.uniform1i(shProgram.useTexture, true);
}
...
function updateGranularity() {
    ...
    const { vertices, indices, normals, texCoords } = CreateSurfaceDataWithIndices(stepsU, stepsV);
    surface.BufferData(vertices, indices, normals, texCoords);
